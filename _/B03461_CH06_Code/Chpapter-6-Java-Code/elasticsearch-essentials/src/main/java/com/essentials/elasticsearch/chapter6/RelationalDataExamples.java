...
//		examples.findChildByParent(parentChildIndexName, parentType, childType);
	...
	private void findNestedDocs(String indexName, String docType, String nestedField) {
		SearchResponse response = client.prepareSearch(indexName)
				.setTypes(docType)
				.setQuery(QueryBuilders.nestedQuery(nestedField, QueryBuilders
						.boolQuery().must(QueryBuilders.matchQuery("tweets.text", "Nosql Databases"))
	...
	private void nestedAggregation(String indexName, String docType, String nestedField) {
		SearchResponse response = client.prepareSearch(indexName)
				.setTypes(docType)
				.addAggregation(AggregationBuilders.nested("NESTED_DOCS").path(nestedField)
